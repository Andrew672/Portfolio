...
@Component({
  ...
  template: `
    <section class="h-full w-full flex flex-col justify-center p-4 md:p-20 relative">
      <h2 i18n class="text-4xl md:text-6xl font-bold text-white mb-8 md:mb-12 border-l-4 border-teal-500 pl-6 w-full max-w-7xl mx-auto">
        Mon parcours
      </h2>
      ...
      <div class="flex justify-center gap-8 mb-8 md:mb-12 w-full max-w-7xl mx-auto">
        <button 
          ...
          [ngClass]="currentView() === 'experience' 
            ? 'text-teal-400 underline decoration-2 decoration-teal-500 underline-offset-4 font-bold' 
      ...
      <div class="relative w-full max-w-7xl mx-auto h-[70vh] group">
        ...
        <div class="h-full overflow-y-auto scrollbar-hide py-8 px-4 md:px-10 space-y-8 mask-gradient" #scrollContainer>
          ...
          <div class="relative min-h-full flex gap-8">
            
            <!-- Frise à gauche -->
            <div class="relative flex flex-col items-center">
              <div class="w-0.5 bg-gradient-to-b from-transparent via-teal-500/30 to-transparent absolute top-0 bottom-0"></div>
              
              @for (exp of currentItems(); track exp.id; let i = $index) {
                <button 
                    (click)="scrollToIndex(i)"
                    class="w-5 h-5 rounded-full z-20 transition-all duration-300 cursor-pointer focus:outline-none border-2 border-slate-900 mb-8 flex-shrink-0"
                    [ngClass]="(selectedItem()?.id === exp.id || exp.current) ? 'bg-teal-400 scale-150' : 'bg-slate-600 hover:bg-teal-500 hover:scale-125 shadow-lg'"
                    [style.margin-top.px]="i === 0 ? 0 : 200">
                </button>
              }
            </div>
            ...
            <div class="flex-1 space-y-8">
              @for (exp of currentItems(); track exp.id; let i = $index) {
                <div #experienceItems class="transition-all duration-500">
